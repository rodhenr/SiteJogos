export const fakeData = {
  user: {
    avatar:
      "https://i.pinimg.com/originals/ba/b5/fe/bab5fe8516e2eead05dcbcb0fe78c102.jpg",
    username: "Usário de teste",
    userID: 0,
    level: 29,
    expLevel: 6000,
    maxExpLevel: 13500,
    messages: [
      {
        username: "Pessoa1",
        userAvatar:
          "https://i.pinimg.com/originals/ba/b5/fe/bab5fe8516e2eead05dcbcb0fe78c102.jpg",
        notRead: 1,
        userID: 1,
      },
      {
        username: "Pessoa2",
        userAvatar:
          "https://i.pinimg.com/originals/ba/b5/fe/bab5fe8516e2eead05dcbcb0fe78c102.jpg",
        notRead: 2,
        userID: 2,
      },
      {
        username: "Pessoa3",
        userAvatar:
          "https://i.pinimg.com/originals/ba/b5/fe/bab5fe8516e2eead05dcbcb0fe78c102.jpg",
        notRead: 0,
        userID: 3,
      },
      {
        username: "Pessoa4",
        userAvatar:
          "https://i.pinimg.com/originals/ba/b5/fe/bab5fe8516e2eead05dcbcb0fe78c102.jpg",
        notRead: 12,
        userID: 4,
      },
    ],
  },
  highlightedGames: [
    {
      name: "Jogo da Velha",
      image:
        "https://store-images.s-microsoft.com/image/apps.2005.14057826194083709.67242c47-4fd7-4f1a-9dd6-5d93f6cc10df.f80f14c0-72ab-46ff-86cd-9d801c8e04e8?mode=scale&q=90&h=300&w=300",
    },
    {
      name: "Yahtzee",
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSOhYrNAU5wvi0KXwAYt-nRm0QdLOh6aESSV17zMbgynoM0GLcnH-gP-p-iEpDl1i3D64s&usqp=CAU",
    },
    {
      name: "JoKenPo",
      image:
        "https://omoldbreaker.files.wordpress.com/2016/05/camiseteria_jokenpo.png?w=499&h=271",
    },
    {
      name: "Uno",
      image:
        "https://cdn2.unrealengine.com/Diesel%2Fproductv2%2Funo%2Fhome%2FGameName_Store_Landscape_2560x1440-2560x1440-5195e8a3e06d672f97a1ee49ecea59027c14cae4.jpg",
    },
  ],
  recentMatches: [
    {
      time: "04/03/2023 12:22",
      game: "Jogo da Velha",
      nameLastUser: "Pessoa3",
      win: true,
    },
    {
      time: "04/03/2023 12:16",
      game: "Yahtzee",
      nameLastUser: "Pessoa4",
      win: false,
    },
    {
      time: "04/03/2023 12:11",
      game: "Uno",
      nameLastUser: "Pessoa1",
      win: false,
    },
    {
      time: "04/03/2023 12:03",
      game: "JoKenPo",
      nameLastUser: "Pessoa2",
      win: true,
    },
  ],
  playerRanking: [
    { position: 1, username: "Pessoa9", level: 29 },
    { position: 2, username: "Pessoa3", level: 28 },
    { position: 3, username: "Pessoa7", level: 27 },
    { position: 4, username: "Pessoa1", level: 27 },
    { position: 5, username: "Pessoa4", level: 26 },
    { position: 6, username: "Pessoa10", level: 25 },
    { position: 7, username: "Pessoa8", level: 24 },
    { position: 8, username: "Pessoa5", level: 24 },
    { position: 9, username: "Pessoa2", level: 24 },
    { position: 10, username: "Pessoa7", level: 23 },
  ],
  gameList: [
    {
      name: "Jogo da Velha",
      image:
        "https://store-images.s-microsoft.com/image/apps.2005.14057826194083709.67242c47-4fd7-4f1a-9dd6-5d93f6cc10df.f80f14c0-72ab-46ff-86cd-9d801c8e04e8?mode=scale&q=90&h=300&w=300",
    },
    {
      name: "Yahtzee",
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSOhYrNAU5wvi0KXwAYt-nRm0QdLOh6aESSV17zMbgynoM0GLcnH-gP-p-iEpDl1i3D64s&usqp=CAU",
    },
    {
      name: "JoKenPo",
      image:
        "https://omoldbreaker.files.wordpress.com/2016/05/camiseteria_jokenpo.png?w=499&h=271",
    },
    {
      name: "Uno",
      image:
        "https://cdn2.unrealengine.com/Diesel%2Fproductv2%2Funo%2Fhome%2FGameName_Store_Landscape_2560x1440-2560x1440-5195e8a3e06d672f97a1ee49ecea59027c14cae4.jpg",
    },
  ],
};
